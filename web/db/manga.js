...
function getManga(mangaId, chapters) {
    const chapterSql = `(SELECT json_agg(ch) FROM 
                            (SELECT title, chapter_number, extract(EPOCH FROM release_date) * 1000 as release_date, "group", service_id, chapter_identifier as chapter_url FROM chapters WHERE manga_id=$1 ORDER BY chapter_number DESC, chapter_decimal DESC NULLS LAST LIMIT $2) ch) 
                         as chapters`;
    const args = [mangaId];
