...
describe('GET /api/quicksearch', () => {
  it('Returns 400 with too small query', async () => {
    await request(httpServer)
      .get('/api/quicksearch')
      .expect(400)
      .expect(expectErrorMessage2('query', 'No search query specified'));

    await request(httpServer)
      .get('/api/quicksearch?query=a')
      .expect(400)
      .expect(expectErrorMessage2('query', 'Query must be between 2 and 500 characters'));
  });
...
describe('GET /api/search', () => {
  it('Returns 400 with too small query', async () => {
    await request(httpServer)
      .get('/api/search')
      .expect(400)
      .expect(res => expect(getErrorMessage2(res, 'query')).toMatchInlineSnapshot(`"No search query specified"`));

    await request(httpServer)
      .get('/api/search?query=a')
      .expect(400)
      .expect(res => expect(getErrorMessage2(res, 'query')).toMatchInlineSnapshot(`"Query must be between 2 and 500 characters"`));
  });
