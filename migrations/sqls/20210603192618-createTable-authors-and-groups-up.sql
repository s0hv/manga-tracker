CREATE TABLE authors (
    ...
    name TEXT NOT NULL,
    mangadex_id UUID UNIQUE DEFAULT NULL
);

CREATE INDEX authors_mangadex_id ON authors(mangadex_id);

-- Table for authors of manga
...
-- Remove old author and artist columns
ALTER TABLE manga_info DROP COLUMN author;
ALTER TABLE manga_info DROP COLUMN artist;



-- Move translation groups to their own table
CREATE TABLE groups (
    group_id SERIAL PRIMARY KEY,
    name TEXT UNIQUE NOT NULL,
    mangadex_id UUID UNIQUE DEFAULT NULL
);
CREATE INDEX groups_mangadex_id ON groups(mangadex_id);

-- Add default group with id 1
INSERT INTO groups (name) VALUES ('No group');

-- Remove chapters with NULL groups as they should not really exist
DELETE FROM chapters WHERE "group" IS NULL;

-- Insert unique groups from chapters
INSERT INTO groups (name) (SELECT DISTINCT "group" FROM chapters);
ALTER TABLE chapters ADD COLUMN group_id INT REFERENCES groups;


-- Move group ids to chapters and set not null constraint
UPDATE chapters c SET group_id=g.group_id
FROM groups as g
WHERE g.name = c."group";
ALTER TABLE chapters ALTER COLUMN group_id SET NOT NULL;

-- Create index for group id
CREATE INDEX chapters_group_id ON chapters(group_id);

-- Create lowercase index for manga titles
CREATE INDEX manga_title_lower_index ON manga(LOWER(title));



DROP FUNCTION IF EXISTS merge_manga(base INTEGER, to_merge INTEGER);
