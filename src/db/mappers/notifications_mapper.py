...
class NotificationsMapper:
    ...
    def chapter_to_notification(
        chapters: list[Chapter], services: dict[int, Service], manga: list[MangaForNotifications]
        ...
        mapped_manga: dict[tuple[int, int], NotificationManga] = {
            (m.manga_id, m.service_id): NotificationsMapper.manga_to_notification(
                m, mapped_services
            )
            for m in manga
        }
        ...
        for chapter in chapters:
            ...
            mapped.append(
                NotificationChapter(
                    ...
                    url=found_manga.service.chapter_url_format.format(
                        chapter.chapter_identifier, title_id=found_manga.title_id
                    ),
    ...
    @staticmethod
    def services_to_notification(
        services: dict[int, Service],
    ) -> dict[int, NotificationMangaService]:
    ...
    @staticmethod
    def manga_to_notification(
        manga: MangaForNotifications, services: dict[int, NotificationMangaService]
    ) -> NotificationManga:
